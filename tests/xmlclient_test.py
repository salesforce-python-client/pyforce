...
class TestBeatbox(unittest.TestCase):
    ...
    def testCreate(self):
        ...
        for k in ['LastName', 'FirstName', 'Phone', 'Email']:
            self.assertEqual(
                data[k], str(contact[getattr(sobjectns, k)]))
    ...
    def testLogout(self):
        ...
        response = svc.logout()
        self.assertEqual(response._name, partnerns.logoutResponse, "Response {} was not {}".format(response._name, partnerns.logoutResponse))
